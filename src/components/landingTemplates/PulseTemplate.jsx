...
import styles from './PulseTemplate.module.css';

/**
 * High-energy register for a barber/blowout-bar type business: a diagonal
 * clip-path cut splits the hero into a color panel and an image panel
 * instead of a straight vertical/horizontal split, and service cards carry
 * an angled accent stripe echoing that same cut. No other template uses
 * clip-path shapes.
 */
export function PulseTemplate({ theme, customer, services }) {
  const navigate = useNavigate();
  const heroEnabled = theme?.heroEnabled !== false;
  const primaryColor = theme?.colors?.primary || '#111827';
  const accentColor = theme?.colors?.accent || '#f97316';
  const { showInNav, showInHero } = businessNameVisibility(theme);
  const activeServices = (services || []).filter((s) => s.active !== false);

  return (
    <div data-testid="landing-template-pulse" className={styles.page} style={{ '--brand': primaryColor, '--accent': accentColor }}>
          ...
          <div className={styles.serviceGrid}>
            {activeServices.map((service) => (
              <div key={service._id} className={styles.serviceCard}>
                <div className={styles.serviceStripe} />
      ...
      <Footer theme={theme} variant="pulse" />
    </div>
  );
}
